...
export function GlobalJobQueue() {
  ...
  const jobs =
    useSupabase("llm_jobs", (q) => q.neq("status", "completed")) || [];
  ...
  const formatJob = (job) => {
    const type = job.type;
    const date = job.meta?.start_date
      ? new Date(job.meta.start_date)
      : job.created_at
    ...
    if (type === "weekly_report")
      return `Weekly (${format(date, "MMM d")}-${format(addDays(date, 6), "MMM d")})`;
    if (type === "monthly_report")
      return `Monthly (${format(date, "MMMM yyyy")})`;
    return "Processing";
  };
