      ...
      <div className="relative z-10 space-y-6">
        <div className="flex items-center justify-between gap-4">
          <h3 className="text-[11px] font-black text-gray-400 uppercase tracking-[0.3em]">
            {type === "monthly_report"
              ? "Monthly Continuity Audit"
              : "Weekly Continuity Audit"}
          </h3>
          {days.length > 7 && (
            <span className="text-[9px] font-bold text-[#818cf8]/60 uppercase tracking-widest animate-pulse">
              Scroll to View →
            </span>
          )}
        </div>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex gap-1.5 px-2">
                {days.map((d) => (
                  <div
                    key={d.toISOString()}
                    className="w-9 flex flex-col items-center gap-1"
                  >
                  ...
                  <div className="flex gap-1.5 shrink-0 px-2">
                    {days.map((day) => {
                      const dayLog = logs?.find(
                        (l) =>
                          l.habit_id === habit.id &&
                          isSameDay(new Date(l.date_string), day),
                      );
                      const isDone = !!dayLog;
                      const isPartial =
                        dayLog &&
                        habit.target_value &&
                        dayLog.value < habit.target_value;
                        ...
                        >
                          {isDone && (
                            <CheckCircle2
                              size={16}
                              className={
                                isPartial ? "text-white/60" : "text-white"
                              }
                            />
                          )}
                        </motion.div>
...
export function Reports() {
  ...
  const latestReport = allReports.find((r) =>
    safeIsSamePeriod(r.start_date || r.created_at, currentDate, activeTab),
  );
  const activeJobs = allJobs.filter(
    (j) =>
      j.status !== "completed" &&
      safeIsSamePeriod(
        j.meta?.start_date || j.created_at,
        currentDate,
        activeTab,
      ),
  );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await requestReport(result.payload, activeTab, {
        start_date: formatISO(currentDate),
      });
      toast.success(
        `${tabs.find((t) => t.id === activeTab).label} analysis queued!`,
      );
    } catch (error) {
  ...
  const parsed = useMemo(() => {
    ...
    if (rhythmSectionMatch) {
      ...
      rhythmLines.forEach((line) => {
        if (line.match(/Peak Performance:/i))
          weeklyRhythm.peak = line
            .replace(/.*Peak Performance:\s*/i, "")
            .trim();
        if (line.match(/Critical Friction:/i))
          weeklyRhythm.friction = line
            .replace(/.*Critical Friction:\s*/i, "")
            .trim();
      });
    }
    ...
    for (let line of lines) {
      ...
      if (
        line.match(
          /^###\s*(Health|Wealth|Finances|Work|Spiritual|Relationships|Social)/i,
        )
      ) {
        ...
        while (
          ...
          if (subLine.match(/^Score:\s*(\d+)/i))
            currentPillar.score = parseInt(subLine.match(/(\d+)/)[1]);
          if (subLine.match(/^Summary:\s*(.*)/i))
            currentPillar.note = subLine.replace(/^Summary:\s*/i, "").trim();
          if (subLine.match(/^Calculation:\s*(.*)/i))
            currentPillar.calculation = subLine
              .replace(/^Calculation:\s*/i, "")
              .trim();
          if (subLine.match(/^Metrics:\s*(.*)/i))
            currentPillar.metrics = subLine.replace(/^Metrics:\s*/i, "").trim();
          if (subLine.match(/^Process:\s*(.*)/i))
            currentPillar.process = subLine.replace(/^Process:\s*/i, "").trim();
          j++;
        }
      ...
      const pillarMatch = line.match(
        /^[-*]\s*(Health|Wealth|Finances|Work|Spiritual|Relationships|Social):\s*(\d+)[^0-9]?\/10\s*[|:-]\s*(.*)$/i,
      );
      if (pillarMatch) {
        let name =
          pillarMatch[1].charAt(0).toUpperCase() +
          pillarMatch[1].slice(1).toLowerCase();
        if (name === "Wealth") name = "Finances";
        if (name === "Relationships") name = "Social";
        ...
        pillars.push({
          ...
          note: pillarMatch[3]
            ? pillarMatch[3].replace(/\)*$/, "").replace(/^\(/, "").trim()
            : "",
        });
        continue;
      }
      ...
      if (line.match(/^#+\s*(Strategic )?Strengths/i)) {
        currentSection = "strengths";
        continue;
      }
      if (line.match(/^#+\s*(Critical )?Weaknesses/i)) {
        currentSection = "weaknesses";
        continue;
      }
      if (line.match(/^#+\s*Cross-Domain Insights/i)) {
        currentSection = "insights";
        continue;
      }
      if (
        line.match(
          /^#+\s*(High-Impact Recommendations|Recommendations|Suggestions)/i,
        )
      ) {
        currentSection = "recommendations";
        continue;
      }
      if (line.match(/^#+\s*(Summary|Executive Summary)/i)) {
    ...
    return {
      ...
      textLines,
      weeklyRhythm,
      score: discoveredScore || latestReport.score || 0,
    };
  }, [latestReport]);
    ...
    <div className="max-w-4xl mx-auto px-4 pt-6 pb-20 space-y-10">
      ...
      <header className="flex flex-col items-center text-center space-y-4">
        <div className="space-y-1">
          <h1 className="text-4xl font-black text-white tracking-tighter glow-text-primary">
            Life Audit
          </h1>
      ...
      <div className="space-y-4 max-w-2xl mx-auto w-full">
        ...
        <div className="flex justify-between items-center glass-panel rounded-2xl p-2 h-14">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <p className="text-[10px] font-black text-[#818cf8] uppercase tracking-[0.2em] mb-0.5">
                Analysis Period
              </p>
              <p className="text-sm font-bold text-gray-200">
                {renderDateLabel(currentDate, activeTab)}
              </p>
            </motion.div>
          </AnimatePresence>
          ...
          <button
            onClick={() => {
              let nextD;
              if (activeTab === "weekly_report")
                nextD = addDays(currentDate, 7);
              else if (activeTab === "monthly_report")
                nextD = addMonths(currentDate, 1);
      ...
      {parsed ? (
        ...
        >
            ...
            <div className="flex flex-col items-center gap-8 relative z-10">
              ...
              <div className="relative w-48 h-48 shrink-0">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <motion.circle
                    ...
                    animate={{
                      strokeDashoffset: 534 - (534 * parsed.score) / 100,
                    }}
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-5xl font-black text-white tracking-tighter glow-text-primary">
                    {parsed.score}
                  </span>
                  <span className="text-[11px] font-black text-gray-500 uppercase tracking-widest mt-1">
                    Life Score
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-4 max-w-2xl px-4">
                ...
                <p className="text-2xl font-bold text-gray-100 leading-snug">
                  {parsed.textLines
                    .find((l) => l.section === "summary")
                    ?.text.replace(/\*/g, "") ||
                    "Analyzing your system trajectory..."}
                </p>
              </div>
            </div>
          </div>
          ...
          {activeTab !== "daily_report" &&
            parsed.weeklyRhythm &&
            (parsed.weeklyRhythm.peak || parsed.weeklyRhythm.friction) && (
              <motion.div
              ...
              >
                <div className="glass-panel p-8 rounded-[32px] bg-emerald-500/[0.02] border-emerald-500/20 relative overflow-hidden group">
                  ...
                  <div className="flex items-center gap-4 mb-4 relative z-10">
                    ...
                    <p className="text-[12px] font-black text-emerald-400 uppercase tracking-[0.25em]">
                      Peak Performance
                    </p>
                  </div>
                ...
                <div className="glass-panel p-8 rounded-[32px] bg-rose-500/[0.02] border-rose-500/20 relative overflow-hidden group">
                  ...
                  <div className="flex items-center gap-4 mb-4 relative z-10">
                    ...
                    <p className="text-[12px] font-black text-rose-400 uppercase tracking-[0.25em]">
                      Critical Friction
                    </p>
                  </div>
                  <p className="text-xl font-bold text-rose-50/90 leading-tight relative z-10">
                    {parsed.weeklyRhythm.friction ||
          ...
          <div className="grid grid-cols-1 gap-6">
            {Object.keys(PILLAR_COLORS).map((pillarName, i) => {
              const p = parsed.pillars.find((x) => x.name === pillarName) || {
                name: pillarName,
                score: 5,
                note: "Insufficient data for active period.",
              };
              return (
                <motion.div
                  key={pillarName}
                ...
                >
                  <div className="absolute top-2 right-6 opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="text-[10px] font-black text-[#818cf8] uppercase tracking-widest bg-[#818cf8]/10 px-2.5 py-1 rounded-full border border-[#818cf8]/20">
                      Detailed Analysis
                    </span>
                  </div>
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-5">
                        <div
                          className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${PILLAR_COLORS[pillarName]} flex items-center justify-center text-white shadow-lg shadow-black/20 shrink-0`}
                        >
          ...
          <div className="space-y-12">
            ...
            <div className="space-y-5">
              ...
              >
                ...
                <div className="space-y-8 relative z-10">
                  ...
                  {parsed.textLines.filter((l) => l.section === "insights")
                    .length === 0 && (
                    <p className="text-gray-600 italic">
                      Identifying patterns across your life systems...
                    </p>
                  )}
                </div>
              </motion.div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col gap-5">
                {parsed.textLines
                  .filter((l) => l.section === "recommendations")
                  .map((line, i) => (
                    <motion.div
                      key={i}
      ...
      <AnimatePresence>
        {selectedPillar && (
          ...
          >
            <div
              className="absolute inset-0 bg-[#0A0A0F]/90 backdrop-blur-xl pointer-events-auto"
            ...
            >
              <div className="p-10 space-y-10">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-6">
                    <div
                      className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${PILLAR_COLORS[selectedPillar.name]} flex items-center justify-center text-white shadow-xl`}
                    >
                      {PILLAR_ICONS[selectedPillar.name]}
                    </div>
                    <div>
                      <h2 className="text-3xl font-black text-white tracking-tighter uppercase">
                        {selectedPillar.name} Audit
                      </h2>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs font-black text-gray-500 uppercase tracking-widest">
                          {selectedPillar.score} / 10 Integrity
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-8">
                  ...
                  <div className="p-8 rounded-3xl bg-white/[0.02] border border-white/5 space-y-3">
                    <p className="text-[10px] font-black text-[#818cf8] uppercase tracking-[0.25em]">
                      Strategic Summary
                    </p>
                  ...
                  <div className="grid grid-cols-1 gap-6">
                    <div className="space-y-3">
                      ...
                      <p className="text-[17px] font-medium text-gray-300 leading-relaxed pl-6 border-l-2 border-blue-400/20">
                        {selectedPillar.calculation ||
                          "Analyzing algorithmic attribution... New reports will include detailed logic."}
                      </p>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="text-[17px] font-medium text-gray-300 leading-relaxed pl-6 border-l-2 border-amber-400/20">
                        {selectedPillar.metrics ||
                          "Identifying data correlation points... New reports will include extracted metrics."}
                      </p>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="text-[17px] font-medium text-gray-300 leading-relaxed pl-6 border-l-2 border-purple-400/20">
                        {selectedPillar.process ||
                          "Tracing systemic reasoning... New reports will include process documentation."}
                      </p>
                    </div>
                  </div>
                </div>
