...
import { useExpensesByDate, addExpense, deleteExpense, updateExpense, useExpensesByRange } from '../hooks/useExpenses';
import { Plus, ChevronLeft, ChevronRight, X, Trash2, Wallet, TrendingDown, CreditCard, Pencil, Calendar, Target, TrendingUp } from 'lucide-react';
import { format, subDays, addDays, isSameDay, isToday as isTodayFn, startOfWeek, endOfWeek, startOfMonth, endOfMonth, subWeeks, addWeeks, subMonths, addMonths } from 'date-fns';
         ...
         <div className="flex justify-between items-end">
            <div>
               <span className="text-[11px] text-emerald-500 uppercase tracking-widest font-black mb-1.5 flex items-center gap-1.5">
                  <TrendingDown size={14} /> {viewType === 'daily' ? 'Daily' : viewType === 'weekly' ? 'Weekly' : 'Monthly'} Burn
               </span>
                  ...
                  >
                     <div className="flex items-center gap-4 flex-1 min-w-0 mr-4">
                        <div className="h-12 w-12 rounded-2xl bg-[#0B0B0F] border border-gray-800 flex items-center justify-center shrink-0">
                           <CreditCard size={20} className={exp.necessity === 'Want' ? 'text-amber-500/70' : 'text-emerald-500/70'} />
                        </div>
                        <div className="flex-1 min-w-0">
                           <div className="flex items-center gap-2">
                              <h3 className="font-bold text-white text-[15px] truncate">{exp.category}</h3>
                              <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-black uppercase tracking-tighter ${exp.type === 'Personal' ? 'bg-gray-800 text-gray-400' : 'bg-indigo-500/10 text-indigo-400'}`}>
                                 {exp.type}
                              </span>
                           </div>
                           <div className="flex items-center gap-1.5 mt-0.5">
                              <p className="text-gray-500 text-xs truncate max-w-[150px]">{exp.description || 'No notes'}</p>
                              <span className="text-gray-800 text-[10px]">•</span>
                              <span className="text-[10px] text-gray-600 font-mono">{new Date(exp.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                           </div>
                        </div>
                     </div>
                     ...
                     <div className="flex items-center gap-3">
                        <div className="text-right">
                           ...
                           <span className={`text-[9px] font-black uppercase tracking-widest ${exp.necessity === 'Want' ? 'text-amber-500' : 'text-emerald-500'}`}>
                              {exp.necessity}
                           </span>
                        </div>
                        <button 
      ...
      <div className="bg-[#12121A] w-full max-w-md max-h-[95vh] overflow-y-auto rounded-[2rem] border border-gray-800 shadow-2xl p-6 sm:p-8 animate-in slide-in-from-bottom-10 sm:slide-in-from-bottom-2">
        ...
        <div className="space-y-6">
           ...
           <div className="space-y-4">
              <div>
                 <label className="block text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-2">Necessity</label>
              ...
              <div>
                 <label className="block text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-2">Allocation Type</label>
           ...
           <div>
              <label className="block text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-2">Note (Optional)</label>
              <input 
                 type="text" 
                 value={description} 
