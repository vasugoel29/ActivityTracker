...
*This script is fully idempotent; it will construct 6 tracking tables, attach B-Tree Indexes explicitly mapped against React query parameters, enforce Row Level Security (RLS) policies allowing Anon access, and natively mount tables into the `supabase_realtime` cross-device publication.*
